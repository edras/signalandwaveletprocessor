...
public abstract class BasicTransform {
   ...
   * domain for a given array depending on the used transform algorithm by
   * inheritance. The number of transformation levels applied is limited by
   * threshold. The approximation vector exactly as detail vector is returned
   * in vector 1.
   * 
   * @date 11.05.2011
   * @author Edras Pacola
   ...
   * @param toLevel
   *          threshold for number of iterations
   * @return coefficients of 1-D frequency or Hilbert domain with approximations
   */
  public abstract HashMap<String, double[]> forward_ext( double[ ] arrTime, int toLevel );
  ...
  public abstract double[ ][ ] wtstnst( double[ ] arrTime, double threshold );
