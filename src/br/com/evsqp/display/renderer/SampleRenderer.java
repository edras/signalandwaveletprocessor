...
/**
 * A renderer that draws a line connecting the Y values to zero
 * {@link XYPlot}.
 */
public class SampleRenderer extends AbstractXYItemRenderer implements
	...
	public void drawItem(Graphics2D g2, XYItemRendererState state,
		...
		XYDataset intervalDataset = (XYDataset) dataset;
		...
		double zero = rangeAxis.valueToJava2D(0, dataArea, yAxisLocation);
		...
		if (orientation == PlotOrientation.HORIZONTAL) {
			line = new Line2D.Double(zero, xx, yy, xx);
			top = ShapeUtilities.createTranslatedShape(shape, yy, xx);
		} else if (orientation == PlotOrientation.VERTICAL) {
			line = new Line2D.Double(xx, zero, xx, yy);
		...
		if (entities != null) {
			...
			XYItemEntity entity = new XYItemEntity(top, dataset, series,
					item, tip, url);
			entities.add(entity);
		}
